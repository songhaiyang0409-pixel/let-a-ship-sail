...
const CAPTURE_ROOT := "res://scenes/staging/believability_pass_02_captures"
const START_Z := 160.0
const END_Z := -175.0
...
func _ready() -> void:
	_configure_viewport()
	capture_mode = OS.get_cmdline_user_args().has("--capture-believability-pass-02")
	...
	print("AUTONOMOUS_WORLD_OCEAN_BELIEVABILITY_PASS_02_READY|route_m=%d|normal_seconds=%.1f|test_seconds=%.1f|v2fun_scale=%.1f|formal_project_modified=false" % [ROUTE_M, ROUTE_M / NORMAL_SPEED, ROUTE_M / TEST_SPEED, BUILDING_SCALE])
	if capture_mode:
...
func _build_world() -> void:
	...
	root.set_meta("asset_status", "AUTONOMOUS_WORLD_OCEAN_BELIEVABILITY_PASS_02")
...
func _build_destination_a() -> void:
	...
	_add_coast_shelf(root, "A_ContinuousTidalShelf", Vector3(0.0, 0.12, 184.0), 72.0, 9.0, 0.7, Color(0.25, 0.29, 0.29, 1.0))
...
func _build_destination_b() -> void:
	...
	_add_quay_wall(root, "B_AttachedStoneQuay", Vector3(5.0, 0.45, -191.5), 17.0, 5.5)
	_add_shore(root, "B_WorkingHarborApron", Vector3(5.0, 0.68, -194.0), Vector3(17.0, 0.24, 8.0), Color(0.28, 0.25, 0.20, 1.0))
	_add_coast_shelf(root, "B_ContinuousShelteredTidalShelf", Vector3(0.0, 0.10, -179.0), 78.0, 10.0, 1.6, Color(0.27, 0.31, 0.30, 1.0))
...
func _build_voyage_context() -> void:
	var root := _new_region("OpenSeaDistanceContext", "quiet lateral coastline and navigation scale")
	# Low, world-space silhouettes sit well outside the sailing lane. Their gaps keep
	# the route calm while giving departure and mid-voyage a measurable sense of scale.
	_add_landform(root, "West_DistantCoast", -78.0, 118.0, -92.0, 8.0, 13.0, 7.5, 1.3, Color(0.19, 0.24, 0.25, 1.0), Color(0.20, 0.30, 0.24, 1.0))
	_add_landform(root, "East_DistantCoast", 80.0, 72.0, -126.0, 7.0, 12.0, 6.0, 3.8, Color(0.18, 0.23, 0.24, 1.0), Color(0.19, 0.29, 0.23, 1.0))
	_add_skerry(root, "WestRouteSkerry", Vector3(-57.0, 0.05, 30.0), Vector3(13.0, 2.8, 19.0), 0.4)
	_add_skerry(root, "EastRouteSkerry", Vector3(61.0, 0.03, -48.0), Vector3(10.0, 2.2, 15.0), 2.2)


func _add_skerry(parent: Node3D, node_name: String, position: Vector3, size: Vector3, phase: float) -> void:
	var root := Node3D.new()
	root.name = node_name + "_DISTANCE_SILHOUETTE"
	...
	for index in range(3):
		...
		mesh.rings = 3
		rock.mesh = mesh
		rock.position = Vector3((float(index) - 1.0) * size.x * 0.22, size.y * (0.28 + 0.09 * index), sin(phase + index) * size.z * 0.12)
		rock.scale = Vector3(size.x * (0.30 + index * 0.04), size.y * (0.46 - index * 0.06), size.z * 0.42)
		rock.material_override = _material(Color(0.20, 0.25, 0.25, 1.0))
		root.add_child(rock)


func _add_coast_shelf(parent: Node3D, node_name: String, center: Vector3, width: float, depth: float, phase: float, color: Color) -> void:
	var vertices := PackedVector3Array()
	var indices := PackedInt32Array()
	var points := 13
	for index in range(points):
		var t := float(index) / float(points - 1)
		var x := (t - 0.5) * width
		var ripple := sin(phase + t * 8.0) * 1.15 + sin(phase * 0.7 + t * 17.0) * 0.35
		vertices.append(center + Vector3(x, 0.0, ripple))
		vertices.append(center + Vector3(x, 0.28 + 0.10 * sin(t * 9.0), depth + ripple * 0.35))
	for index in range(points - 1):
		var i := index * 2
		indices.append_array(PackedInt32Array([i, i + 2, i + 1, i + 1, i + 2, i + 3]))
	var shelf := MeshInstance3D.new()
	shelf.name = node_name + "_DESIGNED_EDGE"
	shelf.mesh = _array_mesh(vertices, indices)
	shelf.material_override = _material(color)
	parent.add_child(shelf)


func _add_quay_wall(parent: Node3D, node_name: String, position: Vector3, width: float, depth: float) -> void:
	var root := Node3D.new()
	root.name = node_name + "_WORKING_STRUCTURE"
	root.position = position
	parent.add_child(root)
	var wall := MeshInstance3D.new()
	var wall_mesh := BoxMesh.new()
	wall_mesh.size = Vector3(width, 1.15, depth)
	wall.mesh = wall_mesh
	wall.position.y = 0.3
	wall.material_override = _material(Color(0.25, 0.27, 0.25, 1.0))
	root.add_child(wall)
	for side in [-1.0, 1.0]:
		var bollard := MeshInstance3D.new()
		var bollard_mesh := CylinderMesh.new()
		bollard_mesh.top_radius = 0.16
		bollard_mesh.bottom_radius = 0.22
		bollard_mesh.height = 0.65
		bollard_mesh.radial_segments = 7
		bollard.mesh = bollard_mesh
		bollard.position = Vector3(side * width * 0.36, 1.0, -depth * 0.28)
...
func _capture_sequence() -> void:
	...
	var captures := [
		...
		["06_harbor_entry_B.png", Vector3(0.0, 0.28, -151.0), 0.0],
		["07_arrival_B.png", Vector3(0.0, 0.28, -174.0), 0.0],
		["08_reverse_view.png", Vector3(0.0, 0.28, -174.0), PI],
	]
	...
	print("BELIEVABILITY_PASS_02_CAPTURE_COMPLETE|route_distance_m=%d|root=%s" % [ROUTE_M, root])
	get_tree().quit()
