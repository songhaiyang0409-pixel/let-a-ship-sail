...
const CAPTURE_ROOT := "res://scenes/staging/reconstruction_03_captures"
const START_Z := 160.0
const END_Z := -175.0
...
func _ready() -> void:
	_configure_viewport()
	capture_mode = OS.get_cmdline_user_args().has("--capture-overnight-world-reconstruction-03")
	...
	print("NORTH_ATLANTIC_RECONSTRUCTION_03_READY|route_m=%d|normal_seconds=%.1f|test_seconds=%.1f|v2fun_scale=%.1f|formal_project_modified=false" % [ROUTE_M, ROUTE_M / NORMAL_SPEED, ROUTE_M / TEST_SPEED, BUILDING_SCALE])
	if capture_mode:
...
func _capture_sequence() -> void:
	...
	print("RECONSTRUCTION_03_CAPTURE_COMPLETE|route_distance_m=%d|root=%s" % [ROUTE_M, root])
	get_tree().quit()
