...
## Candidate A: a single continuous, speed-aware stern trail. It is one generated
## world-space ribbon, not a sequence of decals or white patch sprites.
const LIFETIME := 3.3
const SPACING := 0.30
const MAX_SAMPLES := 72
const STERN_OFFSET := 0.90
const SURFACE_OFFSET := 0.08
const WAVE_TIME_FACTOR := 2.7
const WAKE_TINT := Color(0.64, 0.79, 0.80, 1.0)
...
func _ready() -> void:
	...
	mesh_instance.name = "WakeCandidateA_ContinuousTaperedTrail"
...
func _spawn(speed: float) -> void:
	...
	var point: Vector3 = boat.global_position - forward * STERN_OFFSET
...
func _rebuild() -> void:
	...
	for index in range(points.size()):
		...
		var heading: Vector3 = (points[next_index] - points[previous_index]).normalized()
		if heading.length_squared() < 0.001:
			heading = Vector3.FORWARD
		var right: Vector3 = heading.cross(Vector3.UP).normalized()
		var age: float = 1.0 - life
		var width: float = lerpf(0.10, 0.64, age) * (0.78 + strengths[index] * 0.30)
		width *= 0.96 + sin(float(index) * 1.43) * 0.055
		var center: Vector3 = points[index]
		center.y = _water_y(center) + SURFACE_OFFSET
		var alpha: float = 0.20 * life * life * strengths[index]
		vertices.append(center - right * width)
		vertices.append(center + right * width)
		colors.append(Color(WAKE_TINT.r, WAKE_TINT.g, WAKE_TINT.b, alpha * 0.78))
