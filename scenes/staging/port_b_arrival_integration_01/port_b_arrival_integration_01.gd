...
const CAPTURE_ROOT := "res://scenes/staging/port_b_arrival_integration_01_captures"
const START_Z := 160.0
const END_Z := -175.0
...
func _ready() -> void:
	...
	layout_variant = "b"
	capture_root_override = "res://scenes/staging/port_b_arrival_integration_01_captures/final"
	...
	print("PORT_B_ARRIVAL_INTEGRATION_01_READY|layout=%s|route_m=%d|normal_seconds=%.1f|test_seconds=%.1f|portrait=true|formal_project_modified=false" % [layout_variant, ROUTE_M, ROUTE_M / NORMAL_SPEED, ROUTE_M / TEST_SPEED])
	if harness_mode:
...
func _build_b_dogleg_cove(root: Node3D) -> void:
	...
	_add_landform(root, "B_B_EastInnerArm", 25.0, -176.0, -202.0, 9.0, 13.0, 5.5, 1.9, Color(0.27, 0.31, 0.31, 1.0), Color(0.24, 0.37, 0.27, 1.0))
	_add_shore(root, "B_B_WorkingApron", Vector3(-7.0, 0.39, -194.0), Vector3(8.5, 0.12, 2.6), Color(0.26, 0.25, 0.22, 1.0))
...
func _add_pier(parent: Node3D, node_name: String, position: Vector3) -> void:
	...
	for index in range(3):
		...
		var mesh := BoxMesh.new()
		mesh.size = Vector3(1.25, 0.22, 4.8)
		plank.mesh = mesh
		plank.position = Vector3(-float(index) * 0.65, 0.0, float(index) * -3.8)
		plank.rotation_degrees.y = -8.0
...
func _capture_sequence() -> void:
	...
	for item in stages:
		...
		previous = target
	print("PORT_B_ARRIVAL_INTEGRATION_01_CAPTURE_COMPLETE|route=A_TO_B|stages=9|root=%s|harness=deterministic_in_engine" % root)
	get_tree().quit()
