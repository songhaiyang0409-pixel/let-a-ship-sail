...
## P-18 continuation: long-form crossing atmosphere and Port B approach refinement.
...
const CAPTURE_ROOT := "res://scenes/staging/crossing_atmosphere_harbor_approach_pass_04/captures"
const START_Z := -120.0
const END_Z := -463.0
...
const WORLD_X := 92.0
const WORLD_FRONT := 255.0
const WORLD_BACK := -520.0
...
func _ready() -> void:
	...
	capture_mode = args.has("--capture-p18")
	qa_mode = args.has("--p18-qa")
	harness_mode = capture_mode or qa_mode or args.has("--p18-auto-route")
	layout_variant = "b"
	wake_candidate = "b" if args.has("--terra-wake-b") else DEFAULT_WAKE_CANDIDATE
	capture_root_override = CAPTURE_ROOT
	...
	print("P18_CROSSING_READY|layout=%s|route_m=%d|normal_seconds=%.1f|test_seconds=%.1f|wake_candidate=%s|landscape=true|formal_project_modified=false" % [layout_variant, ROUTE_M, ROUTE_M / NORMAL_SPEED, ROUTE_M / TEST_SPEED, wake_candidate])
	if harness_mode:
...
func _configure_route() -> void:
	...
	ocean.set("transition_01_start", 62.0)
	ocean.set("transition_01_end", 92.0)
	ocean.set("transition_12_start", 152.0)
	ocean.set("transition_12_end", 190.0)
	ocean.set("transition_23_start", 270.0)
	ocean.set("transition_23_end", 305.0)
	ocean.set("coastal_test_mode", false)
	var water := ocean.get("water_mesh") as MeshInstance3D
	if water != null:
		water.scale = Vector3(1.25, 1.0, 8.2)
	...
	if material != null:
		for item in [
			["route_origin_z", -START_Z], ["transition_01_start", 62.0], ["transition_01_end", 92.0],
			["transition_12_start", 152.0], ["transition_12_end", 190.0],
			["transition_23_start", 270.0], ["transition_23_end", 305.0],
		]:
...
func _build_distant_world_context() -> void:
	...
	# Two quiet, offset depth layers replace the old paired horizon lumps. They
	# reveal Port B gradually while leaving the central sailing water uncluttered.
	_add_landform(root, "MidWestWeatherCoast", -119.0, -288.0, -372.0, 21.0, 44.0, 8.5, 1.2, Color(0.20, 0.25, 0.28, 1.0), Color(0.20, 0.30, 0.26, 1.0))
	_add_landform(root, "FarEastLayer", 121.0, -348.0, -448.0, 15.0, 39.0, 6.0, 3.4, Color(0.27, 0.31, 0.32, 1.0), Color(0.25, 0.34, 0.28, 1.0))
	_add_landform(root, "PortB_BackdropLayer", -6.0, -498.0, -535.0, 48.0, 62.0, 9.0, 5.1, Color(0.24, 0.29, 0.30, 1.0), Color(0.25, 0.36, 0.27, 1.0))
	# Sparse asymmetric skerries act as parallax/progress cues, not route arrows.
	_add_rocks(root, "WestPassageSkerries", Vector3(-68.0, 0.22, -238.0), 3)
	_add_rocks(root, "EastApproachSkerries", Vector3(63.0, 0.18, -362.0), 2)
...
func _build_b_offset_inlet(root: Node3D) -> void:
	...
	_add_collision("B_A_LeftLandCollision", Vector3(56.0, 24.0, 56.0), Vector3(-54.0, 11.0, -475.0))
	_add_collision("B_A_RightLandCollision", Vector3(48.0, 22.0, 48.0), Vector3(52.0, 10.0, -477.0))
	_add_collision("B_A_BackLandCollision", Vector3(72.0, 28.0, 24.0), Vector3(0.0, 14.0, -502.0))
func _build_b_dogleg_cove(root: Node3D) -> void:
	...
	_add_collision("B_B_WestLandCollision", Vector3(58.0, 26.0, 60.0), Vector3(-55.0, 12.0, -465.0))
	_add_collision("B_B_EastLandCollision", Vector3(40.0, 22.0, 44.0), Vector3(55.0, 10.0, -486.0))
	_add_collision("B_B_RearLandCollision", Vector3(68.0, 28.0, 25.0), Vector3(10.0, 14.0, -503.0))
func _build_b_asymmetric_headland(root: Node3D) -> void:
	...
	_add_collision("B_C_WestLandCollision", Vector3(62.0, 26.0, 60.0), Vector3(-59.0, 12.0, -467.0))
	_add_collision("B_C_EastLandCollision", Vector3(36.0, 20.0, 44.0), Vector3(53.0, 10.0, -486.0))
	_add_collision("B_C_RearLandCollision", Vector3(70.0, 28.0, 25.0), Vector3(0.0, 14.0, -504.0))
...
func _add_boundaries() -> void:
	_add_collision("WorldWestBoundary", Vector3(1.0, 20.0, 775.0), Vector3(-WORLD_X, 10.0, -132.5))
	_add_collision("WorldEastBoundary", Vector3(1.0, 20.0, 775.0), Vector3(WORLD_X, 10.0, -132.5))
...
func _process(_delta: float) -> void:
	...
	if blocked or outside:
		...
		if not contact_latched:
			print("P18_CONTACT|blocked=%s|outside=%s|position=%s" % [blocked, outside, str(position)])
...
func _land_mask(position: Vector3) -> bool:
	...
	if z >= -430.0 and z <= -415.0:
		return x < -31.0 or x > 34.0
	if z >= -451.0 and z < -430.0:
		return x < -27.0 or x > 15.0
	if z >= -495.0 and z < -451.0:
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo and event.keycode == KEY_BACKSPACE:
		...
		print("P18_RESET|position=%s" % str(last_safe_position))
...
func _capture_sequence() -> void:
	...
	var stages := [
		...
		["02_departure_separation.png", Vector3(0.0, 0.28, -158.0), "wake_review"],
		["03_open_sea_west_skerries.png", Vector3(2.0, 0.28, -208.0)],
		["04_mid_crossing.png", Vector3(0.0, 0.28, -268.0), "wake_review"],
		["05_far_coast_reveal.png", Vector3(0.0, 0.28, -330.0)],
		["06_coastal_approach.png", Vector3(2.0, 0.28, -388.0)],
		["07_harbor_entrance.png", Vector3(-4.0, 0.28, -421.0)],
		["08_dog_leg_turn.png", Vector3(-12.0, 0.28, -438.0), "wake_review"],
		["09_inner_harbor.png", Vector3(-12.0, 0.28, -448.0)],
		["10_working_shore.png", Vector3(-10.0, 0.28, -457.0)],
	...
	for item in stages:
		...
		previous = target
	print("P18_CAPTURE_COMPLETE|route=A_TO_B|stages=11|root=%s|harness=deterministic_in_engine" % root)
	get_tree().quit()
...
func _run_route_qa_and_quit() -> void:
	await _settle(4)
	var routes := {
		"center": [Vector3(0.0, 0.28, -421.0), Vector3(-12.0, 0.28, -433.0), Vector3(-11.0, 0.28, -444.0), Vector3(-8.0, 0.28, -463.0)],
		"moderate_left": [Vector3(-12.0, 0.28, -421.0), Vector3(-18.0, 0.28, -433.0), Vector3(-14.0, 0.28, -444.0), Vector3(-12.0, 0.28, -463.0)],
		"moderate_right": [Vector3(12.0, 0.28, -421.0), Vector3(10.0, 0.28, -433.0), Vector3(4.0, 0.28, -444.0), Vector3(4.0, 0.28, -463.0)],
		"reverse_B_to_A": [Vector3(-8.0, 0.28, -463.0), Vector3(-8.0, 0.28, -444.0), Vector3(0.0, 0.28, -421.0), Vector3(0.0, 0.28, -330.0), Vector3(0.0, 0.28, -220.0), Vector3(0.0, 0.28, START_Z)],
		"slightly_late_turn": [Vector3(0.0, 0.28, -421.0), Vector3(0.0, 0.28, -436.0), Vector3(-22.0, 0.28, -444.0), Vector3(-12.0, 0.28, -463.0)],
		"slightly_early_turn": [Vector3(0.0, 0.28, -421.0), Vector3(-12.0, 0.28, -428.0), Vector3(-12.0, 0.28, -444.0), Vector3(-12.0, 0.28, -463.0)],
		"lateral_safe": [Vector3(14.0, 0.28, -421.0), Vector3(12.0, 0.28, -433.0), Vector3(8.0, 0.28, -444.0), Vector3(4.0, 0.28, -463.0)],
		"excess_lateral_rejected": [Vector3(20.0, 0.28, -421.0), Vector3(20.0, 0.28, -433.0), Vector3(12.0, 0.28, -444.0), Vector3(4.0, 0.28, -463.0)],
	}
	for route_name in routes.keys():
		...
		for index in range(points.size()):
			...
			if _land_mask(point):
				logical_clear = false
			if point.z >= -495.0 and point.z <= -415.0:
