...
func _ready() -> void:
	...
	print("OVERNIGHT_PLAYABLE_WORLD_READY|route_m=%d|normal_seconds=%.1f|test_seconds=%.1f|v2fun_scale=%.1f|formal_project_modified=false" % [ROUTE_M, ROUTE_M / NORMAL_SPEED, ROUTE_M / TEST_SPEED, BUILDING_SCALE])
	if capture_mode:
...
func _build_world() -> void:
	...
	root.set_meta("asset_status", "OVERNIGHT_PRODUCTION_BLOCKOUT")
...
func _build_destination_a() -> void:
	...
	_add_landform(root, "A_West_ExposedSlope", -40.0, 184.0, 232.0, 23.0, 36.0, 18.0, 0.4, Color(0.24, 0.29, 0.31, 1.0), Color(0.19, 0.30, 0.24, 1.0))
	_add_landform(root, "A_East_ExposedSlope", 39.0, 187.0, 228.0, 24.0, 34.0, 15.0, 2.0, Color(0.27, 0.31, 0.32, 1.0), Color(0.22, 0.34, 0.25, 1.0))
	_add_landform(root, "A_WindwardRidge", -4.0, 197.0, 236.0, 13.0, 25.0, 10.0, 3.2, Color(0.29, 0.33, 0.33, 1.0), Color(0.25, 0.37, 0.27, 1.0))
...
func _build_destination_b() -> void:
	...
	_add_landform(root, "B_InhabitedBackSlope", 0.0, -209.0, -236.0, 16.0, 29.0, 10.0, 4.2, Color(0.30, 0.34, 0.33, 1.0), Color(0.33, 0.43, 0.29, 1.0))
	_add_landform(root, "B_HarborWorkingBank", 12.0, -181.0, -206.0, 8.0, 12.0, 3.4, 1.7, Color(0.30, 0.32, 0.31, 1.0), Color(0.31, 0.40, 0.28, 1.0))
	...
	_add_path(root, "B_DockToWarehousePath", Vector3(5.0, 0.65, -188.0), Vector3(15.0, 3.0, -205.0), 1.8, Color(0.35, 0.31, 0.25, 1.0))
...
func _mount_v2fun_assets() -> void:
	...
	if b != null:
		_mount_v2fun(b, SHED, "B_HarborFishingShed", Vector3(15.0, 2.4, -190.0))
...
func _capture_sequence() -> void:
	...
	print("PLAYABLE_WORLD_CAPTURE_COMPLETE|route_distance_m=%d|root=%s" % [ROUTE_M, root])
	get_tree().quit()
