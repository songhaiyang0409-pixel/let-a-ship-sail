...
const VIEWPORT_SIZE := Vector2i(648, 1152)
const CAPTURE_ROOT := "res://scenes/staging/port_b_spatial_01_captures"
const START_Z := 160.0
const END_Z := -175.0
...
func _ready() -> void:
	...
	capture_mode = args.has("--capture-port-b-spatial-01")
	harness_mode = capture_mode or args.has("--port-b-spatial-auto-route")
	if args.has("--port-b-layout-a"):
		layout_variant = "a"
	elif args.has("--port-b-layout-c"):
		layout_variant = "c"
	if args.has("--port-b-final"):
		layout_variant = "b"
		capture_root_override = "res://scenes/staging/port_b_spatial_01_captures/final"
	elif args.has("--port-b-layout-a"):
		capture_root_override = "res://scenes/staging/port_b_spatial_01_captures/layout_a"
	elif args.has("--port-b-layout-c"):
		capture_root_override = "res://scenes/staging/port_b_spatial_01_captures/layout_c"
	else:
		capture_root_override = "res://scenes/staging/port_b_spatial_01_captures/layout_b"
	...
	print("PORT_B_SPATIAL_01_READY|layout=%s|route_m=%d|normal_seconds=%.1f|test_seconds=%.1f|portrait=true|formal_project_modified=false" % [layout_variant, ROUTE_M, ROUTE_M / NORMAL_SPEED, ROUTE_M / TEST_SPEED])
	if harness_mode:
...
func _build_world() -> void:
	...
	root.set_meta("asset_status", "NORTH_ATLANTIC_RECONSTRUCTION_03")
...
func _build_b_dogleg_cove(root: Node3D) -> void:
	...
	_add_landform(root, "B_B_EastInnerArm", 28.0, -176.0, -202.0, 8.0, 13.0, 5.5, 1.9, Color(0.27, 0.31, 0.31, 1.0), Color(0.24, 0.37, 0.27, 1.0))
	_add_shore(root, "B_B_WorkingApron", Vector3(-8.0, 0.48, -194.0), Vector3(17.0, 0.28, 5.0), Color(0.24, 0.22, 0.18, 1.0))
...
func _mount_v2fun_assets() -> void:
	...
	if b != null:
		var shed_x := -8.0 if layout_variant == "b" else 10.0 if layout_variant == "a" else 13.0
		_mount_v2fun(b, SHED, "B_HarborFishingShed", Vector3(shed_x, 2.0, -198.0))
...
func _capture_sequence() -> void:
	...
	var harbor_x := -8.0 if layout_variant == "b" else 10.0 if layout_variant == "a" else 13.0
	var stages := [
		["01_distant_destination_reveal.png", Vector3(0.0, 0.28, 10.0)],
		["02_approach.png", Vector3(0.0, 0.28, -105.0)],
		["03_harbor_entrance.png", Vector3(0.0, 0.28, -151.0)],
		["04_inner_harbor.png", Vector3(harbor_x * 0.55, 0.28, -166.0)],
		["05_working_shoreline.png", Vector3(harbor_x, 0.28, -181.0)],
	]
	...
	for item in stages:
		...
		previous = target
	var arrival := Vector3(harbor_x, 0.28, -193.0)
	await _traverse_to(previous, arrival, 24)
	await _capture_image(root, "06_arrival.png")
	print("PORT_B_SPATIAL_01_CAPTURE_COMPLETE|layout=%s|route_distance_m=%d|root=%s|harness=deterministic_in_engine" % [layout_variant, ROUTE_M, root])
	get_tree().quit()
