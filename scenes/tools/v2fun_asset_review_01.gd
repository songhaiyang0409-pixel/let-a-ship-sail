...
## Isolated front/side/back/game-distance review for V2FUN working derivatives.
## No formal sailing scene, controller, collision, or project settings are used.

const VIEWPORT_SIZE := Vector2i(1152, 648)
const PREVIEW_ROOT := "res://V2FUN_INBOX/previews/asset_review_01"
...
func _mount_asset(asset_path: String, instance_name: String, position: Vector3) -> void:
	...
	if packed == null:
		push_error("Asset review could not load: " + asset_path)
		return
	var instance := packed.instantiate()
	var model := instance as Node3D
	if model == null:
		instance.free()
		push_error("Asset review received a non-Node3D asset: " + asset_path)
		return
...
func _capture_all_views() -> void:
	...
	print("V2FUN_ASSET_REVIEW_COMPLETE|working_scale=%s|capture_root=%s" % [WORKING_SCALE, root])
	get_tree().quit()
