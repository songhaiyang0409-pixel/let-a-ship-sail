...
const CAPTURE_DIR := "res://scenes/water/overnight_v02/port_to_port_slice_02_captures"
...
func _ready() -> void:
	...
	_build_camera()
	_update_wave_visual()
	print("PORT_TO_PORT_SLICE_02_READY|isolated=true|boat_controller=local|water=surface_response_05|hud=false")
	if capture_mode:
...
func _build_port_b() -> void:
	...
	_add_block(shore, "ArrivalDock", Vector3(1.55, 0.18, 6.5), Vector3(-3.0, 0.25, 7.0), Vector3.ZERO, Color(0.31, 0.21, 0.13, 1.0))
	_add_block(shore, "ArrivalDockEnd", Vector3(2.4, 0.20, 0.72), Vector3(-3.0, 0.35, 10.15), Vector3.ZERO, Color(0.42, 0.27, 0.15, 1.0))
...
func _update_navigation(delta: float) -> void:
	...
	if phase == VoyagePhase.ARRIVED and quit_on_arrival:
		print("PORT_TO_PORT_SLICE_02_AUTOPLAY_COMPLETE|phase=ARRIVED|position=%s" % str(boat_movement_root.position))
		get_tree().quit()
...
func _run_collision_check() -> void:
	...
	if failures.is_empty():
		print("PORT_TO_PORT_SLICE_02_COLLISION_CHECK=PASS|PortA=blocked_escape_ok|PortB=blocked|docks=blocked|reset=safe")
	else:
		for failure in failures:
			push_error(failure)
		print("PORT_TO_PORT_SLICE_02_COLLISION_CHECK=FAIL|count=%d" % failures.size())
	get_tree().quit()
...
func _set_phase(new_phase: VoyagePhase) -> void:
	...
	if new_phase == VoyagePhase.ARRIVED:
		boat_speed = 0.0
		print("PORT_TO_PORT_SLICE_02_STATE=ARRIVED")
	else:
		print("PORT_TO_PORT_SLICE_02_STATE=%s" % _phase_name(new_phase))
...
func _capture_all() -> void:
	...
	var shots := [
		{"position": BOAT_START, "phase": VoyagePhase.DEPARTURE, "yaw": 0.0, "wake": 0.0, "file": "01_port_a_start.png"},
		{"position": Vector3(0.0, 0.0, 2.5), "phase": VoyagePhase.OPEN_WATER, "yaw": 0.0, "wake": 0.85, "file": "02_departure.png"},
		...
		{"position": Vector3(0.0, 0.0, -78.0), "phase": VoyagePhase.OPEN_WATER, "yaw": 0.0, "wake": 0.74, "file": "05_port_b_distant.png"},
		{"position": Vector3(0.0, 0.0, -105.0), "phase": VoyagePhase.APPROACH, "yaw": 0.0, "wake": 0.62, "file": "06_port_b_approach.png"},
		{"position": Vector3(0.0, 0.0, -116.5), "phase": VoyagePhase.ARRIVAL, "yaw": 0.0, "wake": 0.34, "file": "07_harbor_entry.png"},
		{"position": Vector3(0.0, 0.0, -119.8), "phase": VoyagePhase.ARRIVED, "yaw": 0.0, "wake": 0.0, "file": "08_arrived.png"},
		{"position": Vector3(0.0, 0.0, -64.0), "phase": VoyagePhase.OPEN_WATER, "yaw": 0.0, "wake": 0.0, "overview": true, "file": "10_world_overview.png"},
	]
	for shot in shots:
