...
## WATER SURFACE RESPONSE 05 - isolated surface-light response test.
##
## Instantiates the existing Robin blockout as a source copy, then swaps only
## the test water shader. The locked 04B wave formula, geometry parameters,
## boat wave-follow, boat visual, island, sky, and cameras are preserved.
...
const CAPTURE_ROOT := "res://scenes/water/water_surface_response_05_captures"
const BEFORE_OVERVIEW := "res://scenes/water/water_color_rescue_04b_captures/01_overview.png"
...
const CAMERA_SHOTS := {
	...
	"surface_response": {"position": Vector3(-2.10, 0.95, 3.90), "target": Vector3(0.0, 0.18, -3.20), "fov": 48.0},
}
...
func _ready() -> void:
	...
	_apply_surface_response_material()
	print("WATER_SURFACE_RESPONSE_05_READY|isolated=true|wave_amplitude=0.55|wave_length=4.60|active=wave_1,wave_5,wave_7,wave_8|ripple=one_directional_light_only|hud=false")
	if OS.get_cmdline_user_args().has("--capture-water-surface-response-05"):
...
func _wait_for_source_nodes() -> void:
	...
	if water_mesh == null or boat_visual == null or camera == null or sun == null:
		push_error("WaterSurfaceResponse05 could not find isolated source nodes.")


func _apply_surface_response_material() -> void:
	if water_mesh == null:
		return
	...
	water_material.set_shader_parameter("ripple_normal_strength", 0.10)
	water_material.set_shader_parameter("ripple_light_strength", 0.22)
	water_material.set_shader_parameter("ripple_specular_strength", 0.45)
...
func _capture_all() -> void:
	...
	var shots := [
		...
		{"name": "surface_response", "file": "05_surface_response.png"},
	]
	...
	for shot in shots:
		...
		if error != OK:
			push_error("Cannot save Water Surface Response 05 screenshot: " + path)
		else:
			print("WATER_SURFACE_RESPONSE_05_SCREENSHOT=" + path)
	...
	print("WATER_SURFACE_RESPONSE_05_SAMPLE|boat_y=%.3f|wave_height=%.3f|amplitude=0.55|length=4.60" % [boat_visual.position.y, float(sample_wave["height"])])
	...
	print("WATER_SURFACE_RESPONSE_05_RENDER_INFO|draw_calls=%s|primitives=%s|gpu_ms_unavailable_in_runtime=true" % [str(draw_calls), str(primitives)])
	get_tree().quit()


func _create_before_after(after_image: Image, output_path: String) -> void:
	var before_image := Image.load_from_file(ProjectSettings.globalize_path(BEFORE_OVERVIEW))
	if before_image == null:
		push_error("Cannot load Water Color Rescue 04B before image.")
		return
	...
	if error != OK:
		push_error("Cannot save Water Surface Response 05 before/after image.")
	else:
		print("WATER_SURFACE_RESPONSE_05_BEFORE_AFTER=" + output_path)
